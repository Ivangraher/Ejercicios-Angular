...
public class JugadorResource {
    ...
    @RequestMapping(value = "/jugadorsycanastas/{canastasTotales}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<Jugador>> getCanastas(@PathVariable Integer canastasTotales) {
        log.debug("REST request to get Jugador : {}", canastasTotales);
        return Optional.ofNullable(jugadorRepository.Canastas(canastasTotales))
            .map(jugador -> new ResponseEntity<>(
                jugador,
                HttpStatus.OK))
            .orElse(new ResponseEntity<>(HttpStatus.NOT_FOUND));
    ...
    @RequestMapping(value = "/jugadorsyequipos/{equiposelected}/{canastasTotales}",
        method = RequestMethod.GET,
        produces = MediaType.APPLICATION_JSON_VALUE)
    @Timed
    public ResponseEntity<List<Jugador>>getEquiposyCanastas(@PathVariable Integer canastasTotales, @PathVariable Long equiposelected) {
        List<Jugador> jugadors =jugadorRepository.Equiposa(canastasTotales,equiposelected);
        return new ResponseEntity<List<Jugador>>(jugadors, HttpStatus.OK);
    }

    public ResponseEntity<Jugador> getEquiposyCanastas(@PathVariable Long id) {
        log.debug("REST request to get Jugador : {}", id);
        return Optional.ofNullable(jugadorRepository.findOne(id))
            .map(jugador -> new ResponseEntity<>(
                jugador,
